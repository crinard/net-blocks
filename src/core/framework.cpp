...
void framework::run_establish_path(builder::dyn_var<connection_t *> conn,
  ...
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
    ...
    if (s == (int)module::hook_status::HOOK_DROP) break;
  }
}
...
builder::dyn_var<int> framework::run_send_path(
    builder::dyn_var<connection_t *> conn, builder::dyn_var<char *> buff,
...
void framework::run_net_init_path(void) {
  runtime::net_state_obj =
      runtime::malloc(runtime::size_of(runtime::net_state_obj));  // TODO: Fix
  for (builder::static_var<unsigned int> i = 0; i < m_registered_modules.size();
       i++) {
